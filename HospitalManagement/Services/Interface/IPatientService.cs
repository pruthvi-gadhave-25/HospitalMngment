...
namespace HospitalManagement.Services.Interface
{
    public interface IPatientService
    {
        ...
        Task<Result<PagedResult<GetPatientDto>>> GetPatientsAsync(int pageIndex = 1, int pageSize = 10,string gender = null);
        Task<Result<GetPatientDto>> GetPatientByIdAsync(int id);
        Task<Result<List<Patient>>> SearchPatientsAsync(string? name);
